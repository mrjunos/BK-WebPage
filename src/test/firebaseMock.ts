...
// Setup global Firestore module mocks
vi.mock("firebase/firestore", () => {
  return {
    getFirestore: vi.fn(() => ({})),
    collection: vi.fn((db, path) => ({ _path: path })),
    query: vi.fn((coll, ...clauses) => coll),
    where: vi.fn((field, op, val) => ({ field, op, val })),
    onSnapshot: vi.fn((queryOrColl, callback) => {
      const path = (queryOrColl as any)._path;
      let docs: any[] = [];

      if (path === "sections") {
        docs = [
          { id: "hero", data: () => mockHero },
          { id: "proceso_intro", data: () => mockProcesoIntro },
          { id: "cta", data: () => mockCta }
        ];
      } else if (path === "chapters") {
        docs = mockChapters.map(ch => ({ id: `ch-${ch.id}`, data: () => ch }));
      } else if (path === "notes") {
        docs = mockNotes.map(nt => ({ id: `nt-${nt.id}`, data: () => nt }));
      } else if (path === "products") {
        docs = mockProducts
          .filter(prod => prod.status === "published")
          .map(prod => ({ id: `prod-${prod.id}`, data: () => prod }));
      }

      // Synchronously call back with our mock snapshot
      callback({
